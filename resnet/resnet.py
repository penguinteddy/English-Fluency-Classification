...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 28 14:57:21 2024

@author: wangkai
try to use resnet to compare with VIT.
"""
...
def train(dataloader, model, loss_fn, optimizer,device):
    ...
    for batch, (X, y) in enumerate(dataloader):#固定格式：batch：第几批数据，不是批次大小，（X，y）：数值用括号
        ...
        print("current batch is: %d" %batch)
        # 统计预测正确的个数(针对分类)
    ...
    print(type(avg_loss))
    #avg_loss_list = [row.tolist() for row in avg_loss] 
    #print(avg_loss,avg_loss_list[0])
    correct /= size
    print(f"correct = {correct}, Train Error: \n Accuracy: {(100 * correct):>0.1f}%, Avg loss: {avg_loss:>8f} \n")
...
if __name__=='__main__':
    ...
    for t in range(epochs):
        ...
        train_accuracy, avg_loss = train(train_dataloader, finetune_net, loss_fn, optimizer, device)
        time_end = time.time()
    ...
    resnet_result['train_loss'] = train_loss
    resnet_result['train_acc'] = train_acc
    resnet_result['test_loss'] = test_loss
    resnet_result['test_acc'] = test_acc
    print("resnet result:", resnet_result)

    plot_history(resnet_result)
